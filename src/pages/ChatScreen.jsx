...
const ChatScreen = ({ chatId, otherUserId, otherUserName, otherUserImage }) => {
      ...
      <div className={`flex mb-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
        ...
        <div className={`max-w-[75%] sm:max-w-xs lg:max-w-md px-3 sm:px-4 py-2 rounded-2xl ${
          ...
          {message.type === 'image' ? (
            ...
            <span className="text-2xl sm:text-3xl">{message.content}</span>
          ) : (
        ...
        {isMe && (
          <div className="ml-1 sm:ml-2 flex-shrink-0">
            <UserAvatar 
              ...
              userName={currentUserData?.fullName || user?.displayName || 'Me'} 
              size="w-6 h-6 sm:w-8 sm:h-8" 
            />
          </div>
        )}
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50" onClick={(e) => {
      ...
      <div className="bg-white border-t p-2 sm:p-4 flex-shrink-0">
        <div className="flex items-center space-x-1 sm:space-x-2">
          ...
          <button
            ...
            className="bg-primary_app text-white p-2 rounded-full hover:bg-opacity-90 disabled:opacity-50"
          >
            {loading ? (
