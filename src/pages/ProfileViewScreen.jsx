...
const ProfileViewScreen = () => {
  ...
  if (!profileData) {
    return (
      <div className="text-center py-8 bg-secondary-dark text-white min-h-screen">
  ...
  return (
    <div className="max-w-2xl mx-auto bg-secondary-dark min-h-screen">
      {/* Back Button */}
      ...
      <div className="p-6 text-center">
          ...
          {profileData.role && (
            <div className="flex items-center justify-center gap-2">
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                profileData.role === 'Veterinarian' ? 'bg-blue-800 text-blue-200' :
                profileData.role === 'Pet Owner' ? 'bg-green-800 text-green-200' :
                'bg-[#313340] text-gray-200'
              }`}>
                {profileData.role}
              </span>
            </div>
        ...
        {privacySettings.showPets !== false && (
          <div className="mt-8">
            <h3 className="text-lg font-bold mb-4 text-white">Pets ({userPets.length})</h3>
            {userPets.length > 0 ? (
              <div className="grid gap-4">
                {userPets.map(pet => (
                  <div key={pet.id} className="bg-[#313340] rounded-lg p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <h4 className="font-semibold text-white">{pet.name || 'Unknown'}</h4>
        ...
        {user?.uid === userId && (
          <button
            ...
            className="bg-[#313340] text-white px-6 py-3 rounded-lg hover:bg-gray-600 flex items-center gap-2 mx-auto"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
