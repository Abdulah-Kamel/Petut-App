    ...
    <div className="max-w-2xl mx-auto bg-white min-h-screen">
      ...
      <div className="p-6 text-center">
          ...
          {profileData.role && (
            <div className="flex items-center justify-center gap-2">
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                profileData.role === 'Veterinarian' ? 'bg-blue-100 text-blue-800' :
                profileData.role === 'Pet Owner' ? 'bg-green-100 text-green-800' :
        ...
        {privacySettings.showPets !== false && (
          <div className="mt-8">
            <h3 className="text-lg font-bold mb-4">Pets ({userPets.length})</h3>
            {userPets.length > 0 ? (
              <div className="grid gap-4">
                {userPets.map(pet => (
                  <div key={pet.id} className="bg-gray-50 rounded-lg p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <h4 className="font-semibold">{pet.name || 'Unknown'}</h4>
        ...
        {user?.uid === userId && (
          <button
            ...
            className="bg-gray-500 text-white px-6 py-3 rounded-lg hover:bg-opacity-90 flex items-center gap-2 mx-auto"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
