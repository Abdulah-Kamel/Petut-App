...
export default function AppointmentsModal({ clinic, modalId, isDarkMode }) {
                ...
                <div className="modal-dialog modal-lg">
                    <div className={`modal-content ${isDarkMode ? 'bg-dark-2 text-white' : ''}`}>
                        <div className="modal-header d-flex align-items-center justify-content-between py-0 pe-0 border-bottom-0">
                            <h1 className="modal-title fs-5 ps-3" id="staticBackdropLabel">Clinic Appointments</h1>
                                    ...
                                    {defaultHours?.length > 0 && (
                                        <ul className="mt-3 list-group w-75">
                                            {defaultHours.map((item, index) => (
                                                <li key={index} className={`list-group-item d-flex justify-content-between align-items-center mb-2 border-2 rounded-4 px-3 py-2 ${isDarkMode ? 'bg-dark text-white border-secondary' : ''}`}>
                                                    <span className={`${isDarkMode ? 'text-white-50' : 'text-gray-500'}`}>{item.day} : {item.openTime} - {item.closeTime}</span>
                        ...
                        <div className="modal-footer d-flex justify-content-end gap-2 border-top-0">
                            <button type="button" className="btn btn-secondary" id='close-btn-edit' data-bs-dismiss="modal" style={{ width: '100px' }}>Close</button>
                        </div>
                    </div>
                </div>
            </div>
